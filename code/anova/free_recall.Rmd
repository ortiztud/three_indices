...
for (c_sub in which_subs){
  ...
  filename = paste(sub_code, "_task-freeRec_merged.csv", sep="")
...
# Fix some R stuff
full_data$cd_rt[full_data$changeness == "no_change"] <- 0
full_data <- full_data %>% filter(OvsN == 1)
...
# Run ANOVA
res.aov <- anova_test(data = data_to_stat, dv = eff_ind, wid = participant, within = congruity)
...
```{r}
# Need to select only the to-be-used columns
data_to_stat <-  subset(x = agg_data, select = c(participant, congruity, mean_id))

# Run ANOVA
res.aov <- anova_test(data = data_to_stat, dv = mean_id, wid = participant, within = congruity)
get_anova_table(res.aov)
...
```{r}
# Run ANOVA
res.aov <- anova_test(data = agg_data, dv = mean_rec, wid = participant, within = congruity)
get_anova_table(res.aov)
...
agg_data <- full_data %>% 
  ...
  filter(cd_acc == 1) %>% 
  group_by(participant, congruity) %>% 
  summarise(mean_fr = sum(fr_acc, na.rm = T)) %>% 
  ungroup()

# Plot
make_plot(agg_data, agg_data$mean_fr, "Mean recognition uracy by congruity")
...
```{r}
# Run ANOVA
res.aov <- anova_test(data = agg_data, dv = mean_fr, wid = participant, within = congruity)
get_anova_table(res.aov)

# Pair-wise comparisons
pwc <- agg_data %>%
  pairwise_t_test(
    mean_fr ~ congruity, paired = TRUE,
    p.adjust.method = "bonferroni"
  )
pwc
