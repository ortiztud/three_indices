...
#### Stats GLMM ####
# Maximal model
max_model<-lmer(cd_rt ~ congruity + 
                   (congruity | sub_code ) +
                   (congruity | obj_file), 
                 data = full_data,
...
# Reduced model (-1)
red_model1a <-lmer(cd_rt ~ congruity + 
                      (congruity | sub_code) +
                      (1 | obj_file), 
                    data = full_data,
                    control=lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
...
red_model1b <-lmer(cd_rt ~ congruity + 
                      (1 | sub_code) +
                      (congruity | obj_file), 
                    data = full_data, 
                    control=lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
...
# in our participants' response to the congruity manipulation. We keep the 
# red_model1b and reduce further
red_model2 <-lmer(cd_rt ~ congruity + 
                     (1 | obj_file), 
                   data = full_data,
                   control=lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
...
ggplot(data = agg_data,
       aes(x = exp, y = cd_rt, color = congruity)) +
  geom_boxplot() + 
