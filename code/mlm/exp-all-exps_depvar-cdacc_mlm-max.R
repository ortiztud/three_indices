...
# Reduced model (-1)
red_model1a <-glmer(cd_acc ~ congruity + 
                      (congruity | sub_code) +
                      (1 | obj_file), 
                    data = full_data, family = binomial, 
                    control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
...
red_model1b <-glmer(cd_acc ~ congruity + 
                      (1 | sub_code) +
                      (congruity | obj_file), 
                    data = full_data, family = binomial, 
                    control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
...
report_title <- "Combined analysis. DV = CD accuracy"
