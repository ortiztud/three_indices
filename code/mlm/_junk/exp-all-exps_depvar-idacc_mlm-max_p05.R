...
# As there is no significant decrease in fit, we keep individual variability 
...
Anova(red_model1b)
#summary(max_model)
red_model1b <-glmer(id_acc ~ congruity * exp+ 
                      (1 | sub_code), 
                    data = full_data, family = binomial, 
                    control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
res.aov <- anova_test(data = agg_data, dv = id_acc, wid = sub_code, within = c(congruity), between = exp)
get_anova_table(res.aov)


#### Aggregate across subjects ####
agg_data <- full_data %>% 
  group_by(exp, congruity) %>% 
