...
# As there is a significant decrease in fit, we keep overall variability 
# in our stimuli. # Now we continue reducing the random effects on the participants side.
red_model1 <-glmer(cd_acc ~ congruity * clutter + 
                      (congruity + clutter | participant) +
                      (1 | obj_file), 
                    data = full_data, family = binomial, 
...
# As there is no significant decrease in fit from either congruity or clutter,
# we reduce more
...
# Now we can compare the models. 
anova(red_model2a, red_model3)
anova(red_model2b, red_model3)

# As there is no significant decrease in fit, we accept the model without any random
# slopes for our participants (red_model3)
# Create summary table for the model selection process
source('/home/javier/git_repos/premup/analysis/report_LMM.R')
tested_models <- c(max_model,red_model1,red_model2a,red_model2b,red_model3)
model_names <- c("Maximal model","Reduced 1","Reduced 2a","Reduced 2b","Reduced 3","Reduced 4a","Reduced 4b")
against_models <- c(1,2,2,3)
report_title <- "Experiment 1a. Change detection MLM"
