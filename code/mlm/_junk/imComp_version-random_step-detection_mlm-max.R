...
# Removing the random effect of clutter on the stimuli does not reduce the fit
...
# variability in our stimuli's reaction to the congruity manipulation (red_model2a).
...
# As there is no significant decrease in fit, we discard individual variability 
# in our participant's reaction to the congruity manipulation. We reduce more.
red_model5 <-glmer(cd_acc ~ congruity * clutter + 
                      (1 | participant) +
                      (congruity | obj_file), 
                    data = full_data, family = binomial, 
                    control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
...
# Now we can compare the models. 
anova(red_model4a, red_model5)
anova(red_model4b, red_model5)

# As there is no significant decrease in fit, we select the model without any random
# slopes for our participants (red_model5)
