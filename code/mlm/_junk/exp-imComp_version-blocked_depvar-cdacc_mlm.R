...
# variability in our stimuli's overall accuracy scores.
# Now we continue reducing the random effects on the participants side.
red_model2 <-glmer(cd_acc ~ congruity * clutter + 
                      (congruity + clutter | participant) +
                     (1 | obj_file), 
                    data = full_data, family = binomial, 
                    control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
...
red_model3a <-glmer(cd_acc ~ congruity * clutter + 
...
red_model3b <-glmer(cd_acc ~ congruity * clutter + 
                      (clutter | participant) +
                      (1 | obj_file),  
                    data = full_data, family = binomial, 
                    control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)))
