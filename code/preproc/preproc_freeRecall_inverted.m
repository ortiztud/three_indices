...
for cSub=which_sub
    ...
    cd.RTCong(cSub,:)=[nanmean(p.RTcd(p.trialType==1 & p.targetCong==1 & p.accuracyCd==1)), ...
        nanmean(p.RTcd(p.trialType==1 & p.targetCong==2 & p.accuracyCd==1))];
    cd.RT(cSub,:)=[
...
for cSub=which_sub
    ...
    % Store cd acc and rt separately
    cd_acc(:,cSub) = p.accuracyCd;
    cd_rt(:,cSub) = p.RTcd;
    cd_cong(:,cSub) = p.targetCong;
...
% Compute cd trial averages
% cd_rt(isnan(cd_rt))
for c_sub = which_sub
    acc_sub(c_sub,:)=[mean(cd_acc(cd_cong(:,c_sub)==1,c_sub)), mean(cd_acc(cd_cong(:,c_sub)==2,c_sub))];
    ind = isnan(cd_rt(:,c_sub));
    cd_rt(ind,c_sub) = 6;
    rt_sub(c_sub,:)=[mean(cd_rt(cd_cong(:,c_sub)==1,c_sub)), mean(cd_rt(cd_cong(:,c_sub)==2,c_sub))];

    % Efficiency
    eff_sub(c_sub,1) = acc_sub(c_sub,1) / rt_sub(c_sub,1);
    eff_sub(c_sub,2) = acc_sub(c_sub,2) / rt_sub(c_sub,2);
end
